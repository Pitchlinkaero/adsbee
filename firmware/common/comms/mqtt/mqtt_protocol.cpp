...
static inline const char* BandStr(MQTTProtocol::FrequencyBand band) {
    return (band == MQTTProtocol::BAND_UAT) ? "uat" : "adsb";
}

static inline const char* BandShort(MQTTProtocol::FrequencyBand band) {
    return (band == MQTTProtocol::BAND_UAT) ? "u" : "a";
}
...
bool MQTTProtocol::GetTopic(uint32_t icao24,
                            ...
                            bool short_form) {
    if (!out_topic || !msg_type || !device_id || out_size < 16) return false;
    const char* band_str = short_form ? BandShort(band) : BandStr(band);
    if (short_form) {
        // {device_id}/a|u/{ICAO}/s|r
        const char* t = (strcmp(msg_type, "status") == 0) ? "s" : "r";
        int n = snprintf(out_topic, out_size, "%s/%s/%06X/%s", device_id, band_str, (unsigned)icao24, t);
        return (n > 0 && n < out_size);
    } else {
        // {device_id}/adsb|uat/{ICAO}/status|raw
        int n = snprintf(out_topic, out_size, "%s/%s/%06X/%s", device_id, band_str, (unsigned)icao24, msg_type);
        return (n > 0 && n < out_size);
    }
}

bool MQTTProtocol::GetSystemTopic(const char* msg_type,
                                  char* out_topic,
                                  uint16_t out_size,
                                  const char* device_id,
                                  bool short_form) {
    if (!out_topic || !msg_type || !device_id || out_size < 12) return false;
    if (short_form) {
        // {device_id}/sys/t|g
        const char* t = (strcmp(msg_type, "telemetry") == 0) ? "t" : (strcmp(msg_type, "gps") == 0) ? "g" : "on";
        int n = snprintf(out_topic, out_size, "%s/sys/%s", device_id, t);
        return (n > 0 && n < out_size);
    } else {
        // {device_id}/system/telemetry|gps|online
        int n = snprintf(out_topic, out_size, "%s/system/%s", device_id, msg_type);
        return (n > 0 && n < out_size);
    }
}

static const char* GetCatCode(uint8_t category_raw) {
    static char buf[4];
    uint8_t ca = (category_raw >> 5) & 0x07;
    uint8_t ty = category_raw & 0x1F;
    char letter = 'A';
    switch (ca) {
    ...
    snprintf(buf, sizeof(buf), "%c%u", letter, (unsigned)ty);
    return buf;
}

uint16_t MQTTProtocol::FormatAircraftStatusJSON(const Aircraft1090& ac,
                                                FrequencyBand band,
                                                char* out_json,
                                                uint16_t out_size) {
    if (!out_json || out_size < 32) return 0;
    const char* band_val = (band == BAND_UAT) ? "BAND_UAT" : "BAND_1090";
    int n = snprintf(out_json, out_size,
                     "{\"icao\":\"%06X\",\"band\":\"%s\",\"call\":\"%s\",\"cat\":\"%s\"," 
                     "\"lat\":%.5f,\"lon\":%.5f,\"alt_ft\":%ld,\"hdg_deg\":%.0f,\"spd_kts\":%.0f,"
                     "\"vr_fpm\":%d,\"sqk\":\"%04X\",\"on_ground\":%d}",
                     (unsigned)ac.icao_address,
                     band_val,
                     ac.callsign,
                     GetCatCode(ac.category_raw),
                     ac.latitude_deg,
                     ac.longitude_deg,
                     ac.baro_altitude_ft,
                     ac.direction_deg,
                     ...
                     ac.squawk,
                     (ac.flags & (1 << Aircraft1090::kBitFlagIsAirborne)) ? 0 : 1);
