#ifndef ADSBEE_MQTT_PROTOCOL_HH_
#define ADSBEE_MQTT_PROTOCOL_HH_

#include <stdint.h>
#include <stddef.h>
#include "data_structures.hh"  // For Aircraft1090
...
public:
    ...
    static const uint16_t kMaxTopicLen = 64;
    static const uint16_t kMaxJSONLen = 512;

    // Build per-aircraft topic per SPEC.md. Returns true on success.
    static bool GetTopic(uint32_t icao24,
                         const char* msg_type,   // "status" | "raw"
                         char* out_topic,
                         uint16_t out_size,
                         FrequencyBand band,
    ...
    // Build system topic (telemetry/gps). Returns true on success.
    static bool GetSystemTopic(const char* msg_type,   // "telemetry" | "gps" | "online"
                               char* out_topic,
                               uint16_t out_size,
                               const char* device_id,  // 16 hex chars
                               bool short_form = false);

    // JSON formatters (proto3 JSON mapping fields as per contracts/mqtt/mqtt_messages.proto)
    static uint16_t FormatAircraftStatusJSON(const Aircraft1090& ac,
                                             FrequencyBand band,
                                             char* out_json,
                                             uint16_t out_size);
};

#endif  // ADSBEE_MQTT_PROTOCOL_HH_
